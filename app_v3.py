...
#                  (username TEXT, image_path TEXT, result TEXT)''')
#     conn.commit()

# def add_to_search_history(username, image_path, result):
#     c.execute('INSERT INTO search_history (username, image_path, result) VALUES (?, ?, ?)', (username, image_path, result))
#     conn.commit()

# def get_search_history(username):
#     c.execute('SELECT image_path, result FROM search_history WHERE username = ?', (username,))
#     return c.fetchall()
...
#             add_to_search_history(st.session_state.username, uploaded_file_path, prediction_label)
#     except Exception as e:
...
#         for image_path, result in search_history:
...
def display_search_history():
    try:
        ...
        for image_path, result, probability in search_history:
            st.image(Image.open(image_path), caption=f"Result: {result}", use_column_width=True)
            st.write(f"Probability of result: {probability:.2f}%")
            st.write("---")
    except Exception as e:
        st.error(f"An error occurred while fetching search history: {str(e)}")
